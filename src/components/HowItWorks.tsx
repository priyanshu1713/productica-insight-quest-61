...
const HowItWorks = () => {
  return (
    <section className="section-container relative bg-gradient-to-b from-productica-dark to-productica-black">
      <div className="absolute inset-0 opacity-40">
        <div className="h-full w-full bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-white/5 via-transparent to-transparent"></div>
      </div>
      
      {/* Decorative Elements */}
      <div className="absolute top-20 right-10 w-64 h-64 bg-white/5 rounded-full blur-3xl opacity-20"></div>
      <div className="absolute bottom-20 left-10 w-80 h-80 bg-white/5 rounded-full blur-3xl opacity-20"></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {steps.map((step, index) => (
            <div key={index} className="relative group">
              <div className="glass-card rounded-lg p-6 h-full transition-all duration-300 group-hover:bg-white/5">
                <div className="text-white text-4xl font-bold mb-4 opacity-50 group-hover:opacity-80 transition-opacity">{step.number}</div>
              ...
              {index < steps.length - 1 && (
                <div className="hidden lg:block absolute top-1/2 -right-3 transform -translate-y-1/2 z-10">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-white group-hover:scale-110 transition-transform">
